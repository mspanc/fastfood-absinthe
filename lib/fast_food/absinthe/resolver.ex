defmodule FastFood.Absinthe.Resolver do
  ...
  def resolve_root_query_many(ecto_schema, parent, args, resolution) do
    Logger.debug(
      "[FastFood.Absinthe.Resolver] Root Query (many), ecto_schema = #{inspect(ecto_schema)}, parent = #{inspect(parent)}, args = #{inspect(args)}"
    )
  ...
  def resolve_root_query_one(ecto_schema, parent, %{id: id} = args, resolution) do
    Logger.debug(
      "[FastFood.Absinthe.Resolver] Root Query (one), ecto_schema = #{inspect(ecto_schema)}, parent = #{inspect(parent)}, args = #{inspect(args)}"
    )
  ...
  def resolve_root_create_one(ecto_schema, parent, %{input: input} = args, resolution) do
    Logger.debug(
      "[FastFood.Absinthe.Resolver] Root Create (one), ecto_schema = #{inspect(ecto_schema)}, parent = #{inspect(parent)}, args = #{inspect(args)}"
    )
  ...
  def resolve_root_update_one(ecto_schema, parent, %{id: id, input: input} = args, resolution) do
    Logger.debug(
      "[FastFood.Absinthe.Resolver] Root Update (one), ecto_schema = #{inspect(ecto_schema)}, parent = #{inspect(parent)}, args = #{inspect(args)}"
    )
  ...
  def resolve_root_delete_one(ecto_schema, parent, %{id: id} = args, resolution) do
    Logger.debug(
      "[FastFood.Absinthe.Resolver] Root Delete (one), ecto_schema = #{inspect(ecto_schema)}, parent = #{inspect(parent)}, args = #{inspect(args)}"
    )
  ...
  defp do_load_single(
       ...
       when is_atom(assoc_field_type) and length(selections) != 0 do
