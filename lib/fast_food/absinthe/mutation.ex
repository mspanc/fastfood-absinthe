defmodule FastFood.Absinthe.Mutation do
  ...
  defp make_create_one(ecto_schema) do
    ...
    IO.puts "Root Mutation (create one): ecto_schema = #{inspect(ecto_schema)}, absinthe_input_type = #{inspect(absinthe_input_type)}, absinthe_return_type = #{inspect(absinthe_return_type)}, mutation_name = #{inspect(mutation_name)}"
    quote do
  ...
  defp make_update_one(ecto_schema) do
    ...
    IO.puts "Root Mutation (update one): ecto_schema = #{inspect(ecto_schema)}, absinthe_input_type = #{inspect(absinthe_input_type)}, absinthe_return_type = #{inspect(absinthe_return_type)}, mutation_name = #{inspect(mutation_name)}"
    quote do
      field unquote(mutation_name), unquote(absinthe_return_type) do
        arg :id, non_null(:id)
        arg :input, non_null(unquote(absinthe_input_type))
  ...
  defp make_delete_one(ecto_schema) do
    ...
    IO.puts "Root Mutation (delete one): ecto_schema = #{inspect(ecto_schema)}, absinthe_input_type = #{inspect(absinthe_input_type)}, absinthe_return_type = #{inspect(absinthe_return_type)}, mutation_name = #{inspect(mutation_name)}"
    quote do
      field unquote(mutation_name), unquote(absinthe_return_type) do
        arg :id, non_null(:id)
        resolve fn(parent, args, resolution) ->
